...
const RecentlyAddedItem = React.createClass({
	getInitialState:function(){
		return({
			design:this.props.design,
...
const RecentlyAdded = React.createClass({
	getInitialState:function(){
		return ({
			recentlyAdded:DesignStore.getRecentlyAdded(5)
		})
	},
	componentDidMount:function(){
	...
	render:function(){
		...
		if(recentlyAdded != null) {
			var recentItems = recentlyAdded.map(function(design,idx){
				return <RecentlyAddedItem design={design} key={design.hash} />
			});
		} else {
			return null;
		}
		return (
			<div id="recently-added">
				<div id="title">Recently Added</div>
				<div id="item-holder">
					{recentItems}
					<div className="cb"></div>
				</div>
